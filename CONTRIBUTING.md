...
- `project/`：Go Core、SwiftUI macOS 客户端、Electron/React Windows 客户端、打包脚本和回归测试。
...
- Node.js 22+ 与 npm（Electron/React Windows 客户端）
...
如果修改 macOS Keychain、进程、App 或打包逻辑，请运行：
...
并核对 `lipo -info`、`codesign --verify --deep --strict`、包内 `SHA256SUMS.txt`
以及 Apple Silicon 实机 `doctor`。Intel 切片必须明确区分“交叉编译通过”和“Intel 实机通过”。
...
4. 不降低 loopback、PKCE、DPAPI、macOS Keychain、随机本地密钥和配置备份等安全边界。
...
然后重新验证本地补丁、两个 Go 模块以及 Windows/macOS 发布包。
