...
func TestFileTokenStoreSaveExistingMetadataSetsFileAttributes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			decoded, errDecode := unmarshalAuthMetadataFromStorage(persisted)
			if errDecode != nil {
				t.Fatalf("decode saved auth file: %v", errDecode)
			}
			expected := []byte(`{"type":"antigravity","access_token":"` + tt.savedToken + `","disabled":false}`)
			decodedJSON, errMarshal := json.Marshal(decoded)
			if errMarshal != nil {
				t.Fatalf("marshal decoded auth file: %v", errMarshal)
			}
			if !jsonEqual(decodedJSON, expected) {
				t.Errorf("decoded saved auth file = %s, want JSON equal to %s", decodedJSON, expected)
			}
		})
	}
}
