...
type malformedAuthJSONError struct {
	err error
}

func (e *malformedAuthJSONError) Error() string { return "unmarshal auth json: " + e.err.Error() }
func (e *malformedAuthJSONError) Unwrap() error { return e.err }
...
func (s *FileTokenStore) Save(ctx context.Context, auth *cliproxyauth.Auth) (string, error) {
	...
	case auth.Metadata != nil:
		auth.Metadata["disabled"] = auth.Disabled
		raw, errMarshal := marshalAuthMetadataForStorage(auth.Metadata)
		if errMarshal != nil {
			return "", fmt.Errorf("auth filestore: marshal metadata failed: %w", errMarshal)
		}
		if existing, errRead := os.ReadFile(path); errRead == nil {
			if existingMetadata, errDecode := unmarshalAuthMetadataFromStorage(existing); errDecode == nil && deepEqualJSON(existingMetadata, auth.Metadata) {
				break
			}
			if errWrite := writeAuthFileAtomic(path, raw, 0o600); errWrite != nil {
				return "", fmt.Errorf("auth filestore: replace existing failed: %w", errWrite)
			}
			break
...
func (s *FileTokenStore) List(ctx context.Context) ([]*cliproxyauth.Auth, error) {
	...
	err := filepath.WalkDir(dir, func(path string, d fs.DirEntry, walkErr error) error {
		...
		if errReadAuths != nil {
			var malformed *malformedAuthJSONError
			if errors.As(errReadAuths, &malformed) {
				return fmt.Errorf("auth filestore: load %s: %w", path, errReadAuths)
			}
			return nil
		}
		if len(auths) > 0 {
...
func (s *FileTokenStore) readAuthFiles(path, baseDir string) ([]*cliproxyauth.Auth, error) {
	...
	metadata, err := unmarshalAuthMetadataFromStorage(data)
	if err != nil {
		return nil, &malformedAuthJSONError{err: err}
	}
	needsMigration, errMigrationCheck := authMetadataStorageNeedsMigration(data)
	if errMigrationCheck != nil {
		return nil, fmt.Errorf("inspect auth storage: %w", errMigrationCheck)
	}
	if needsMigration {
		protectedData, errMarshal := marshalAuthMetadataForStorage(metadata)
		if errMarshal != nil {
			return nil, fmt.Errorf("protect legacy auth json: %w", errMarshal)
		}
		if errWrite := writeAuthFileAtomic(path, protectedData, 0o600); errWrite != nil {
			return nil, fmt.Errorf("migrate legacy auth json: %w", errWrite)
		}
		data = protectedData
	}
