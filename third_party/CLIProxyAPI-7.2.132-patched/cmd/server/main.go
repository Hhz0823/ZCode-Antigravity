...
func main() {
	...
	var standalone bool
	var localModel bool
	var skipAntigravityVersionUpdate bool
	...
	flag.BoolVar(&skipAntigravityVersionUpdate, "skip-antigravity-version-update", false, "Use the embedded Antigravity client version and skip the background version lookup")
	...
	if vertexImport != "" {
		...
		if tuiMode {
			if standalone {
				...
				if !skipAntigravityVersionUpdate {
