...
func (s *GitTokenStore) ensureRepositoryLocked() error {
	...
	if _, err := os.Stat(gitDir); errors.Is(err, fs.ErrNotExist) {
			...
			if errors.Is(errClone, transport.ErrEmptyRemoteRepository) {
				...
				defer func() {
					if errClose := repo.Close(); errClose != nil {
						log.WithError(errClose).Warn("git token store: close initialized repository")
					}
				}()
				if s.branch != "" {
		...
		defer func() {
			if repo != nil {
				if errClose := repo.Close(); errClose != nil {
					log.WithError(errClose).Warn("git token store: close repository")
				}
			}
		}()
		...
		if errVerify := verifyRepositoryHead(repo); errVerify != nil {
			...
			errRecover := s.recoverRepositoryLocked(repoDir, authMethod, repo, nil, nil)
			repo = nil
			if errRecover != nil {
				s.dirLock.Unlock()
		...
		if errPull := worktree.Pull(pullOpts); errPull != nil {
			...
			case errors.Is(errPull, git.NoErrAlreadyUpToDate):
				if errReset := resetIndexToHead(repo, worktree); errReset != nil {
					...
					errRecover := s.recoverRepositoryLocked(repoDir, authMethod, repo, prePullTree, dirtyPaths)
					repo = nil
					if errRecover != nil {
						s.dirLock.Unlock()
...
func (s *GitTokenStore) guardWatcherAuthRemovalLocked(message string, relPaths []string) (bool, error) {
	...
	defer func() {
		if errClose := repo.Close(); errClose != nil {
			log.WithError(errClose).Warn("git token store: close watcher guard repository")
		}
	}()
...
func disableGitCommitSigning(repoDir string) error {
	...
	defer func() {
		if errClose := repo.Close(); errClose != nil {
			log.WithError(errClose).Warn("git token store: close config repository")
		}
	}()
...
func (s *GitTokenStore) recoverRepositoryLocked(repoDir string, authMethod []client.Option, localRepo *git.Repository, baselineTree *object.Tree, dirtyPaths map[string]struct{}) (errRecovery error) {
	if localRepo == nil {
		return fmt.Errorf("recover repository: local repository is nil")
	}
	localRepoOpen := true
	defer func() {
		if localRepoOpen {
			if errClose := localRepo.Close(); errClose != nil {
				errClose = fmt.Errorf("close local repository after recovery: %w", errClose)
				if errRecovery == nil {
					errRecovery = errClose
				} else {
	...
	if baselineTree == nil {
		inspectedTree, inspectedDirtyPaths, errInspect := inspectRecoveryBaseline(localRepo)
		if errInspect != nil {
	...
	defer func() {
		if clonedRepoOpen {
			if errClose := clonedRepo.Close(); errClose != nil {
				errClose = fmt.Errorf("close cloned repository after recovery: %w", errClose)
				if errRecovery == nil {
					errRecovery = errClose
				} else {
					errRecovery = errors.Join(errRecovery, errClose)
	...
	if errClose := clonedRepo.Close(); errClose != nil {
		return fmt.Errorf("close recovered clone before install: %w", errClose)
	}
	clonedRepoOpen = false
	if errClose := localRepo.Close(); errClose != nil {
		return fmt.Errorf("close corrupt repository before backup: %w", errClose)
	}
	localRepoOpen = false
	...
	if errOpen == nil {
		...
		if errClose := recoveredRepo.Close(); errClose != nil {
			errClose = fmt.Errorf("close verified recovered repository: %w", errClose)
			if errOpen == nil {
				errOpen = errClose
			} else {
				errOpen = errors.Join(errOpen, errClose)
			}
		}
	}
	if errOpen != nil {
...
func inspectRecoveryBaseline(repo *git.Repository) (*object.Tree, map[string]struct{}, error) {
	if repo == nil {
		return nil, nil, fmt.Errorf("open repository: repository is nil")
	}
	worktree, errWorktree := repo.Worktree()
	if errWorktree != nil {
...
func (s *GitTokenStore) commitAndPushWithOptionsLocked(message string, allowMissingRemote bool, relPaths ...string) error {
	...
	defer func() {
		if errClose := repo.Close(); errClose != nil {
			log.WithError(errClose).Warn("git token store: close commit repository")
		}
	}()
...
func (s *GitTokenStore) maybeRunGC(repoDir string) {
	...
	defer func() {
		if errClose := repo.Close(); errClose != nil {
			log.WithError(errClose).Warn("git token store: close gc repository")
		}
	}()
