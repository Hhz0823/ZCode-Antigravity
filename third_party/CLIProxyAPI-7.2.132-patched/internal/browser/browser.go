...
func OpenURL(url string) error {
	fmt.Println("Attempting to open the authentication URL in a browser")

	if configured := strings.TrimSpace(os.Getenv("CLIPROXY_BROWSER")); configured != "" {
		if !filepath.IsAbs(configured) {
			return fmt.Errorf("CLIPROXY_BROWSER must be an absolute path")
		}
		info, errStat := os.Stat(configured)
		if errStat != nil || info.IsDir() {
			return fmt.Errorf("CLIPROXY_BROWSER is not an executable file: %s", configured)
		}
		if errStart := exec.Command(configured, "--new-window", url).Start(); errStart != nil {
			return fmt.Errorf("start configured browser: %w", errStart)
		}
		return nil
	}
