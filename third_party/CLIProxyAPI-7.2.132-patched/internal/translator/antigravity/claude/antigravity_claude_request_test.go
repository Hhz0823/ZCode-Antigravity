...
func TestConvertClaudeRequestToAntigravity_AudioAndVideoContent(t *testing.T) {
	inputJSON := []byte(`{
	...
	audio := gjson.GetBytes(output, "request.contents.0.parts.0.inlineData")
	if audio.Get("mimeType").String() != "audio/wav" || audio.Get("data").String() != "UklGRg==" {
		t.Fatalf("audio inlineData mismatch: %s", audio.Raw)
	}
	video := gjson.GetBytes(output, "request.contents.0.parts.1.inlineData")
	if video.Get("mimeType").String() != "video/mp4" || video.Get("data").String() != "AAAAIGZ0eXA=" {
		t.Fatalf("video inlineData mismatch: %s", video.Raw)
	}
}
