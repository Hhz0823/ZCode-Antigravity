...
func TestApplyThinkingWithSourcePayloadMapsZCodeGemini37Effort(t *testing.T) {
	for _, level := range []string{"low", "medium", "high"} {
		t.Run(level, func(t *testing.T) {
			source := []byte(`{"model":"gemini-3.7-flash-high","thinking":{"type":"adaptive"},"output_config":{"effort":"` + level + `"}}`)
			...
			if got := gjson.GetBytes(out, "request.generationConfig.thinkingConfig.thinkingLevel").String(); got != level {
				t.Fatalf("thinkingLevel = %q, want %q; body=%s", got, level, out)
			}
		})
	}
}
