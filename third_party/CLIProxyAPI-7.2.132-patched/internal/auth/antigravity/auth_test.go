...
func TestFetchProjectIDRetriesCompletedOnboardingUntilProjectPropagates(t *testing.T) {
	attempts := 0
	auth := NewAntigravityAuth(nil, &http.Client{Transport: roundTripperFunc(func(req *http.Request) (*http.Response, error) {
		switch req.URL.String() {
		case "https://cloudcode-pa.googleapis.com/v1internal:loadCodeAssist":
		...
		case "https://daily-cloudcode-pa.googleapis.com/v1internal:onboardUser":
			...
			return jsonResponse(`{"done":true,"response":{"cloudaicompanionProject":{"id":"propagated-project"}}}`), nil
		default:
			t.Fatalf("unexpected request URL: %s", req.URL.String())
			return nil, nil
		}
	...
	if projectID != "propagated-project" || attempts != 2 {
		t.Fatalf("projectID=%q attempts=%d", projectID, attempts)
	}
}
