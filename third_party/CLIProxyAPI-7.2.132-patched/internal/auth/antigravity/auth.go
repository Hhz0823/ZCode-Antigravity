...
import (
	...
	"github.com/router-for-me/CLIProxyAPI/v7/internal/auth/antigravitycredentials"
...
func (o *AntigravityAuth) BuildAuthURL(state, redirectURI string, pkceCodes *PKCECodes) (string, error) {
	if pkceCodes == nil || strings.TrimSpace(pkceCodes.CodeChallenge) == "" {
		return "", fmt.Errorf("antigravity auth URL: PKCE challenge is required")
	}
	oauthClient, errCredentials := antigravitycredentials.Load()
	...
	params.Set("client_id", oauthClient.ID)
...
func (o *AntigravityAuth) ExchangeCodeForTokens(ctx context.Context, code, redirectURI string, pkceCodes *PKCECodes) (*TokenResponse, error) {
	if pkceCodes == nil || strings.TrimSpace(pkceCodes.CodeVerifier) == "" {
		return nil, fmt.Errorf("antigravity token exchange: PKCE verifier is required")
	}
	oauthClient, errCredentials := antigravitycredentials.Load()
	if errCredentials != nil {
	...
	data := url.Values{}
	data.Set("code", code)
	data.Set("client_id", oauthClient.ID)
	data.Set("client_secret", oauthClient.Secret)
		...
		if resp.StatusCode == http.StatusOK {
			...
			if done, okDone := data["done"].(bool); okDone && done {
				...
				if attempt == maxAttempts {
					return "", fmt.Errorf("project id unavailable after onboarding; the account may be ineligible or restricted")
				}
				// A completed onboarding response can precede project propagation.
