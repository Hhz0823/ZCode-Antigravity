...
**当前版本：** Windows / macOS `v0.4.7-test`
...
- 主窗口、Windows 任务栏小组件与 macOS 菜单栏小组件采用 [codexU](https://github.com/shanggqm/codexU) 的浅灰蓝背景、白色圆角卡片、顶部品牌栏、胶囊切换与分层指标结构，并以 Rust / Win32 和 SwiftUI / AppKit 独立实现。
...
| 文件 | 用途 | 建议 |
| --- | --- | --- |
| `ZCode-Antigravity-macOS-Universal-v0.4.7-test.zip` | macOS Universal App 与维护脚本 | **Mac 用户首选** |
| `ZCode-Antigravity-Setup-v0.4.7-test.exe` | Windows 图形化安装器 | **普通用户首选** |
| `ZCode-Antigravity-OneClick-v0.4.7-test.bat` | 内嵌完整运行包的单文件安装器 | 备用方案 |
| `ZCode-Antigravity-Windows-x64-0.4.7-test.zip` | 可展开、可逐文件校验的便携包 | 手动部署 / 排错 |
| `ZCode-Antigravity-Source-v0.4.7-test.zip` | 当前源码快照 | 审计 / 构建 |
...
3. 下载 `ZCode-Antigravity-Setup-v0.4.7-test.exe`，并先校验 SHA-256。Windows 请勿使用点击操作缺少反馈且 OAuth 过期后额度刷新失败的 `v0.4.3-test`、界面退化且高 DPI 字体异常的 `v0.4.2-test`，也不要继续使用更早测试版。
...
Windows 基线与 `v0.4.7-test` 新增的构建及实机测试记录包括：
...
- Rust Win32 控制中心使用 codexU 风格顶部品牌栏、胶囊提供商切换、白色悬浮卡片与 Segoe UI Variable 字体；按真实显示器 DPI 创建窗口并在 `WM_DPICHANGED` 时重建字体。
...
- `v0.4.7-test` 在同一实机显示账号摘要、最低 98%、分组额度进度条和重置时间；状态与 Token 统计每 5 秒更新，但不会改写独立的 5 分钟额度缓存周期。
...
- `v0.4.7-test` SwiftUI 原生界面已实机启动；codexU 风格主窗口、明暗/系统主题按钮、5 分钟刷新标识、提供商切换、状态卡和自定义菜单栏额度弹层显示正常。
