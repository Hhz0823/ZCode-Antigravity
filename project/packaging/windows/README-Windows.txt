ZCode Antigravity Bridge - Windows x64 stable build
===================================================

Purpose
-------
This package lets ZCode and other local agents use Antigravity Gemini, Google-account Claude, or xAI Grok text
...
ZCode-Antigravity-Setup-v1.0.3.exe. This is a native Windows GUI installer: it shows no
...
required OAuth desktop configuration. v1.0.3 also repairs a recorded gateway automatically
...
ZCode-Antigravity-OneClick-v1.0.3.bat. It verifies and extracts its embedded package,
...
7. Reopen ZCode. Select Provider "Google" and choose the desired
   Gemini, Google Claude, or Grok text model. Google Claude is opt-in under Settings and uses the
   existing Antigravity Google account without a separate Anthropic API key. The system-tray quota
   widget remains available after closing the panel.
...
Automatic updates
-----------------
The control center checks the latest stable GitHub Release about three seconds after startup and
then every six hours. Settings can check manually, install an available release immediately, or
enable automatic installation. Automatic installation is off by default.

Only the exact ZCode-Antigravity-Setup asset for the reported stable version is accepted. The Go
Core verifies the GitHub SHA-256 while downloading; Electron restricts the installer to the current
user update directory, resolves the real path, rejects symbolic links, and calculates SHA-256 again.
ZCode must be fully exited before installation. Update mode deploys without a success dialog, starts
the new control center with --post-update, and resynchronizes the local gateway without opening ZCode.

Known Google Claude availability on 2026-09-04
------------------------------------------------
The tested Antigravity Google account returned claude-sonnet-4-6 and
claude-opus-4-6-thinking. Both declare a 200,000-token input limit, 64,000-token output limit,
text/image input, and text output. Enable "Google Claude / other AI text models" in Settings,
apply the model switches, then run Test-Claude-Sonnet-4.6.bat for a small real request.
...
- Test-Claude-Sonnet-4.6.bat    Google-account Claude Sonnet 4.6 inference and audit artifact
...
Installer formats
-----------------
- ZCode-Antigravity-Setup-v1.0.3.exe: recommended no-terminal current-user installer.
- ZCode-Antigravity-OneClick-v1.0.3.bat: fallback single-file installer.
- ZCode-Antigravity-Windows-x64-1.0.3.zip: manually verifiable expanded package.
...
requests remain normal chat/coding requests. Google Claude appears only when the optional model switch is enabled
and the logged-in Antigravity account returns it. Grok exposes the text models returned for the logged-in xAI account.
Imagine image/video models and unrelated providers are excluded. Missing required Gemini or enabled Grok text models
cause a clear setup error instead of silently substituting another model.

Build versions
--------------
- ZCode Antigravity Bridge: 1.0.3
