...
func requiredTestModels() []modelInfo {
	return []modelInfo{{
		...
		ID:                        zcodeWebSearchModelID,
		DisplayName:               "Gemini Web Search (Google)",
		MaxInputTokens:            1048576,
		SupportedInputModalities:  []string{"text", "image", "audio", "video"},
		SupportedOutputModalities: []string{"text"},
	}}
...
func TestWriteBackendConfigRestrictsNetworkAndCredits(t *testing.T) {
	...
	checks := []string{
		...
		`alias: "gemini-web-search"`,
		`display-name: "Gemini Web Search (Google)"`,
		"force-mapping: true",
		a.apiKey,
	}
...
func TestSelectZCodeModelsAllowsGeminiFlashAndWebSearch(t *testing.T) {
	...
	if isAllowedZCodeModel("claude-opus-4-6-thinking") || isAllowedZCodeModel("gemini-3.8-flash-high") || isAllowedZCodeModel("gemini-3.7-flash-high") || !isAllowedZCodeModel("gemini-3.8-flash") || !isAllowedZCodeModel("gemini-3.7-flash") || !isAllowedZCodeModel("gemini-3.6-flash") || !isAllowedZCodeModel(zcodeWebSearchModelID) {
...
func TestZCodeModelAliasesMatchClientAllowlist(t *testing.T) {
	...
	for _, alias := range zcodeModelAliases {
		isSearchAlias := alias.ClientID == zcodeWebSearchModelID && alias.UpstreamID == "gemini-3.1-flash-lite"
		isCleanFlashAlias := strings.HasSuffix(alias.UpstreamID, "-high") && !strings.HasSuffix(alias.ClientID, "-high")
		if alias.UpstreamID == alias.ClientID || (!isSearchAlias && !isCleanFlashAlias) {
...
func TestSelectZCodeModelsRequiresBothModels(t *testing.T) {
	if _, err := selectZCodeModels(requiredTestModels()[:1]); err == nil || !strings.Contains(err.Error(), "gemini-3.7-flash") {
		t.Fatalf("expected missing Gemini models error, got %v", err)
	}
}
...
`)
	...
	gemini38 := configuredModels["gemini-3.8-flash"].(map[string]any)
	limit38 := gemini38["limit"].(map[string]any)
	if fmt.Sprint(limit38["context"]) != "1048576" {
		t.Fatalf("Gemini 3.8 context limit = %v", limit38["context"])
	}
	modalities38 := gemini38["modalities"].(map[string]any)
	if fmt.Sprint(modalities38["input"]) != "[text image audio video]" {
		t.Fatalf("Gemini 3.8 input modalities = %v", modalities38["input"])
	}
	reasoning38 := gemini38["reasoning"].(map[string]any)
	if reasoning38["enabled"] != true || fmt.Sprint(reasoning38["variants"]) != "[low medium high]" || reasoning38["defaultVariant"] != "high" {
		t.Fatalf("Gemini 3.8 reasoning selector = %#v", reasoning38)
	}
