...
func TestPreferredConnectorModelHonorsConfiguredBackgroundModel(t *testing.T) {
	models := []modelInfo{{ID: "gemini-3.8-flash"}, {ID: "gemini-3.7-flash"}}
	if got := preferredConnectorModel("antigravity", models, "gemini-3.7-flash"); got != "gemini-3.7-flash" {
		t.Fatalf("preferred model = %q", got)
	}
	if got := preferredConnectorModel("xai", []modelInfo{{ID: "grok-build-0.1"}}, "gemini-3.7-flash"); got != "grok-build-0.1" {
		t.Fatalf("xai preferred model = %q", got)
	}
	if got := preferredConnectorModel("antigravity", models, ""); got != "gemini-3.8-flash" {
		t.Fatalf("newest fallback model = %q", got)
	}
}
