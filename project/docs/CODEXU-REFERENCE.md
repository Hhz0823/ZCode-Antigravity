...
ZCode Antigravity `v0.4.7-test` 的主窗口、任务栏 / 菜单栏小组件、额度信息层级与
推理性能口径参考了 [shanggqm/codexU](https://github.com/shanggqm/codexU) 的公开界面和产品说明：
使用浅灰蓝背景、白色 18px 圆角卡片、顶部品牌栏、胶囊切换、蓝紫数据色和分层指标结构；
常驻小组件直接显示 5 小时 / 7 天额度、重置时间、输出 Token 与 Token/s。
...
ZCode Antigravity 的 Rust + Win32 与 SwiftUI + AppKit 代码独立重建。ZCode Antigravity
直接读取现有 CLIProxyAPI 的脱敏 usage 记录：有 TTFT 时以输出 Token 除以首字节后的生成阶段
耗时并标为“生成速度”；缺少 TTFT 时才使用完整调用时长，并明确标为“有效吞吐”。Windows
任务栏弹层由 Rust + Win32 构建，macOS 菜单栏弹层由 SwiftUI + AppKit 构建。
